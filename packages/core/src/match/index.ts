...
function calculateTeamStrength(players: Player[], tactics: Tactics): number {
  ...
  const totalOverall = lineupPlayers.reduce((sum, p) => sum + calculateOverall(p), 0)
  const avgOverall = totalOverall / lineupPlayers.length

  // Adjust for tactical posture
  const postureBonus = tactics.posture === 'attacking' ? 3 : tactics.posture === 'defensive' ? -3 : 0
...
function calculateChanceSuccess(
  ...
  const defenseStrength = calculateTeamStrength(defendingPlayers, { ...attackingTactics, posture: 'defensive' })
...
function pickScorer(players: Player[], tactics: Tactics): Player | undefined {
  ...
  const weights = attackers.map((p) => p.attributes.shooting + p.attributes.positioning)
...
function simulateMinute(state: MatchState, config: MatchConfig): void {
  ...
  const attackingPlayers = attackingTeam === 'home' ? state.homeLineup : state.awayLineup
  const defendingPlayers = attackingTeam === 'home' ? state.awayLineup : state.homeLineup
  const attackingTactics = attackingTeam === 'home' ? state.homeTactics : state.awayTactics
  ...
  if (eventRoll < 0.4) {
    // Attacking chance
    const successChance = calculateChanceSuccess(attackingPlayers, defendingPlayers, attackingTactics)
    ...
    // Yellow card
    const allPlayers = attackingTeam === 'home' ? state.awayLineup : state.homeLineup
...
export function simulateMatch(config: MatchConfig): MatchResult {
  ...
  const state: MatchState = {
    ...
    awayScore: 0,
    events: [],
    homeLineup: homeTeam.players.filter((p) => homeTactics.lineup.includes(p.id)),
    awayLineup: awayTeam.players.filter((p) => awayTactics.lineup.includes(p.id)),
    homeSubs: homeTeam.players.filter((p) => homeTactics.substitutes.includes(p.id)),
    awaySubs: awayTeam.players.filter((p) => awayTactics.substitutes.includes(p.id)),
    homeTactics,
    awayTactics,
    possession: 'home',
  }
...
export function* simulateMatchLive(config: MatchConfig): Generator<{ minute: number; state: MatchState; event?: MatchEvent }> {
  const { homeTeam, awayTeam, homeTactics, awayTactics } = config
  ...
  for (let minute = 0; minute <= totalMinutes; minute++) {
    ...
    const newEvent = state.events.length > eventsBefore ? state.events[state.events.length - 1] : undefined
