...
import type { Team, Tactics, Season, Fixture, FormationType, TacticalPosture } from '@retrofoot/core'
import {
  generateSquad,
  selectBestLineup,
...
function getPlayerTeam(teams: Team[], playerTeamId: string | null): Team | null {
  ...
  persist(
    (set, get) => ({
      ...
      autoSelectLineup: () => {
        ...
        const { lineup, substitutes } = selectBestLineup(team, tactics.formation)
        set({
          tactics: {
            ...tactics,
            lineup,
...
export function useUpcomingFixture(): {
  ...
  if (!season || !playerTeamId) {
    return { fixture: null, homeTeam: null, awayTeam: null, isPlayerHome: false }
  }

  const roundFixtures = getCurrentRoundFixtures(season)
  const fixture = roundFixtures.find(
